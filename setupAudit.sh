...
# Script to generate AWS cost audit boilerplate files non-interactively

# Define root directory
ROOT_DIR="aws-cost-audit-boilerplates"
...
# Create scripts/generate_cost_audit.py
echo "Creating generate_cost_audit.py..."
cat << 'EOF' > "$ROOT_DIR/scripts/generate_cost_audit.py"
import boto3
import os
import json
...
# Create reports/cost_audit_report.md (template)
echo "Creating cost_audit_report.md..."
cat << 'EOF' > "$ROOT_DIR/reports/cost_audit_report.md"
...
FILES=(
  "$ROOT_DIR/scripts/generate_cost_audit.py"
  "$ROOT_DIR/reports/cost_audit_report.md"
)
...
echo "AWS Cost Audit boilerplate files created successfully in $ROOT_DIR/"
echo "Next steps:"
echo "1. Install dependencies for generate_cost_audit.py:"
echo "   pip install boto3"
echo "2. Configure AWS credentials (AWS CLI or environment variables)."
echo "3. Replace placeholders in reports/cost_audit_report.md (e.g., {{AWS_REGION}}, {{SNS_TOPIC_ARN}}) or set environment variables:"
echo "   export AWS_REGION=us-east-1"
echo "   export SNS_TOPIC_ARN=arn:aws:sns:us-east-1:123456789012:alerts"
echo "4. Run the audit script to generate the report:"
echo "   python3 $ROOT_DIR/scripts/generate_cost_audit.py"
echo "5. Review the generated report at $ROOT_DIR/reports/cost_audit_report.md."
echo "6. Schedule the script using AWS Lambda and EventBridge for periodic audits:"
echo "   - Package the Python script with dependencies."
...
cat << 'EOF'
   ...
   ```
EOF
echo "8. Push to GitHub: Initialize a repo, add files, and share with the community."
exit 0
